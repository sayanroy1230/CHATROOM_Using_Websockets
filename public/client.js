...
const append = (message, position) => {
    ...
    if(position=='left'){
            ...
            timedisplay.classList.add('left');
            timedisplay.classList.add('lefttime');
            msgcontainer.append(timedisplay);
        }
    ...
    if(position=='right'){
            time=new Date()
            console.log(`${time.getHours()}:${time.getMinutes()}`);
            timedisplay1 = document.createElement('div');
            timedisplay1.innerHTML=`${time.getHours()}:${time.getMinutes()}`;
            timedisplay1.classList.add('right');
            timedisplay1.classList.add('righttime');
            msgcontainer.append(timedisplay1);
        }
